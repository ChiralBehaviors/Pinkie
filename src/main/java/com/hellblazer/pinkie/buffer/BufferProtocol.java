...
final public class BufferProtocol {
		...
		public void readReady() {
			...
			if (readBuffer.hasRemaining()) {
				if (readFullBuffer) {
					if (log.isDebugEnabled()) {
						log.debug(
								"socket {} read buffer still needs bytes, selecting for read",
								socketInfo());
					}
		...
		public String socketInfo() {
			...
			try {
				return String.format("[%s local=%s remote=%s]",
						channel.isConnected() ? "connected" : "unconnected",
		...
		public void writeReady() {
			...
			if (writeBuffer.hasRemaining()) {
				if (writeFullBuffer) {
					if (log.isDebugEnabled()) {
						log.debug(
								"socket {} write buffer still has bytes, selecting for write",
								socketInfo());
					}
					handler.selectForWrite();
				} else {
	...
	private static final Logger log = LoggerFactory
			.getLogger(BufferProtocol.class);
